...
C_TEMP = {'c_alpha': 0.3, 'c_beta': 2.0, 'c_gamma': -3.3}
C_RH = {'c_alpha': 0.4, 'c_beta': 2.0, 'c_gamma': -3.4}
C_WIND = {'c_alpha': 0.3, 'c_beta': 3.5, 'c_gamma': -3.3}

def make_prediction(fcsts, c_alpha, c_beta, c_gamma, v='TEMP', change_at='SUNSET', min_value=float('-inf'), max_value=float('inf'), intervals=1, verbose=False):
    if verbose:
    ...
    fcsts['VAR_MIN_TOM'] = pd.concat([fcsts[var_min][1:], pd.Series([fcsts[var_min].iloc[-1]])]).values
    # FIX: simplify in R code too
    # FIX: R code seems to use NA instead of a value at the end?
    fcsts['TIME_MIN_TOM'] = pd.concat([fcsts['TIME_MIN'][1:], pd.Series([fcsts['TIME_MIN'].iloc[-1]])]).values
    ...
    cross['TIME'] = cross.apply(lambda row: row['HOUR'] + row['MINUTE'] / 60.0, axis=1)
    for column in ['HOUR', 'MINUTE', 'TIME', 'MON', 'YEAR']:
        if column in fcsts.columns:
            del fcsts[column]
    cross = pd.merge(cross, fcsts, on=['ID', 'DATE'])
    cross['START_DATE'] = cross.apply(lambda row: (pd.to_datetime(row['DATE']) - datetime.timedelta(days=1)).strftime('%Y-%m-%d') if row['TIME'] <= row['TIME_MIN'] else row['DATE'], axis=1)
    cross['HOUR_CURVE'] = cross.apply(lambda row: (row['TIME'] + 24) if row['TIME'] <= row['TIME_MIN'] else row['TIME'], axis=1)
    cross['IS_RISING'] = cross.apply(lambda row: False if row['TIME'] <= row['TIME_MIN'] else row['TIME'] < row[change_at], axis=1)
    cross['TIME_G_MIN'] = cross.apply(lambda row: row['TIME_MIN'] if row['TIME'] <= row['TIME_MIN'] else row['TIME_MIN_TOM'], axis=1)
    cross['VAR_G_MIN'] = cross.apply(lambda row: row[var_min] if row['TIME'] <= row['TIME_MIN'] else row['VAR_MIN_TOM'], axis=1)
    rising = cross[cross['IS_RISING'] == True][:]
    falling = cross[cross['IS_RISING'] == False][:]
    # figure out values before change_at
    rising['F_OR_G'] = rising.apply(lambda row: (row['HOUR_CURVE'] - row['TIME_MIN']) / (row['TIME_MAX'] - row['TIME_MIN']), axis=1)
    rising[v] = rising.apply(lambda row: (row[var_min] + (row[var_max] - row[var_min]) * sin((pi / 2) * row['F_OR_G'])), axis=1)
    rising['VAR_CHANGE'] = rising.apply(lambda row: (row[var_min] + (row[var_max] - row[var_min]) * sin((pi / 2) * ((row[change_at] - row['TIME_MIN']) / (row['TIME_MAX'] - row['TIME_MIN'])))), axis=1)
    ...
    falling['F_OR_G'] = falling.apply(lambda row: (row['HOUR_CURVE'] - row[change_at]) / (24 - row[change_at] + row['TIME_G_MIN']), axis=1)
    falling[v] = falling.apply(lambda row: row['VAR_G_MIN'] + (row['VAR_CHANGE'] - row['VAR_G_MIN']) * exp(c_gamma * row['F_OR_G']), axis=1)
    # combine and sort everything
    out = pd.concat([rising, falling])
    out = out.sort_values(['DATE', 'HOUR'])
    out['TIMESTAMP'] = out.apply(lambda row: pd.to_datetime(f"{row['DATE']} {row['HOUR']}:{row['MINUTE']}:00"), axis=1)
...
def do_prediction(fcsts, row_temp, row_wind, row_rh, intervals=1, verbose=False):
    ...
    v_temp = make_prediction(fcsts, row_temp['c_alpha'], row_temp['c_beta'], row_temp['c_gamma'], 'TEMP', 'SUNSET', intervals=intervals, verbose=verbose)
    v_wind = make_prediction(fcsts, row_wind['c_alpha'], row_wind['c_beta'], row_wind['c_gamma'], 'WIND', 'SUNSET', min_value=0, intervals=intervals, verbose=verbose)
    # FIX: R version only looks at HOUR so wouldn't work with intervals != 1
    t = v_temp[['ID', 'TIMESTAMP', 'DATE', 'HOUR', 'MINUTE', 'LAT', 'LONG', 'TEMP']]
    w = v_wind[['ID', 'TIMESTAMP', 'WIND']]
    out = pd.merge(t, w)
    rh = make_prediction(fcsts, row_rh['c_alpha'], row_rh['c_beta'], row_rh['c_gamma'], 'RH_OPP', intervals=intervals, min_value=0, max_value=1, verbose=verbose)
    rh['RH'] = rh['RH_OPP'].apply(lambda rh_opp: 100 * (1 - rh_opp))
    rh = rh[['ID', 'TIMESTAMP', 'RH']]
    out = pd.merge(out, rh)
    output = out[['ID', 'LAT', 'LONG', 'TIMESTAMP', 'TEMP', 'WIND', 'RH']][:]
    if verbose:
        print('Assigning times')
    output['HOUR'] = output['TIMESTAMP'].apply(lambda x: x.hour)
    ...
    rain = fcsts[['DATE', 'RAIN']][:]
    rain['HOUR'] = 7
    rain['MINUTE'] = 0
    if verbose:
        print('Merging')
    cmp = pd.merge(output, rain, how='outer')
    cmp['RAIN'] = cmp['RAIN'].fillna(0)
    if verbose:
        print('Calculating times')
    cmp['YEAR'] = cmp['TIMESTAMP'].apply(lambda x: x.year)
    cmp['MON'] = cmp['TIMESTAMP'].apply(lambda x: x.month)
    cmp['DAY'] = cmp['TIMESTAMP'].apply(lambda x: x.day)
    cmp['TIME'] = cmp.apply(lambda row: row['HOUR'] + row['MINUTE'] / 60.0, axis=1)
    if verbose:
...
def minmax_to_hourly_single(w, timezone, skip_invalid=False, verbose=False):
    ...
    if 1 != len(r['YEAR'].unique()):
        raise RuntimeError('Expected a single YEAR value for input weather')
    r['TIMESTAMP'] = r.apply(lambda row: datetime.datetime(int(row['YEAR']), int(row['MON']), int(row['DAY']), int(row['HOUR'])), axis=1)
    if not(len(r) > 1 and util.is_sequential_days(r)):
        if skip_invalid:
            raise RuntimeWarning(f'{r["ID"].iloc[0]} for {r["YEAR"].iloc[0]} - Expected input to be sequential daily weather')
        return None
    orig_dates = pd.DataFrame({'date': map(lambda x: x.strftime('%Y-%m-%d'), r['TIMESTAMP'].apply(lambda x: x.date()).unique())})
    ...
    r['YEAR'] = r['TIMESTAMP'].apply(lambda x: x.year)
    r['MON'] = r['TIMESTAMP'].apply(lambda x: x.month)
    r['DAY'] = r['TIMESTAMP'].apply(lambda x: x.day)
    r['HOUR'] = r['TIMESTAMP'].apply(lambda x: x.hour)
    dates = r['TIMESTAMP'].unique()
    latitude = r['LAT'].iloc[0]
    longitude = r['LONG'].iloc[0]
    r[['SOLRAD', 'SUNRISE', 'SUNSET']] = r.apply(lambda row: util.sun(latitude, longitude, row['MON'], row['DAY'], row['HOUR'], timezone), axis=1, result_type='expand')
    # # FIX: is solar noon just midpoint between sunrise and sunset?
    r['SOLARNOON'] = r.apply(lambda row: (row['SUNSET'] - row['SUNRISE']) / 2 + row['SUNRISE'], axis=1)
...
def minmax_to_hourly(w, timezone, skip_invalid=False, verbose=False):
    ...
    for stn in r['ID'].unique():
        by_stn = r[r['ID'] == stn]
        for yr in by_stn['YEAR'].unique():
            by_year = by_stn[by_stn['YEAR'] == yr]
