...
def make_prediction(
    ...
    out["TIMESTAMP"] = out.apply(
        lambda row: pd.to_datetime(f"{row['DATE']} {row['HOUR']}:{row['MINUTE']}:00"),
        axis=1,
    )
...
def minmax_to_hourly_single(w, timezone, skip_invalid=False, verbose=False):
    ...
    r["TIMESTAMP"] = r.apply(
        lambda row: datetime.datetime(
            int(row["YEAR"]), int(row["MON"]), int(row["DAY"]), int(row["HOUR"])
        ),
        axis=1,
    )
    if not (len(r) > 1 and util.is_sequential_days(r)):
        if skip_invalid:
            raise RuntimeWarning(
                f'{r["ID"].iloc[0]} for {r["YEAR"].iloc[0]} - Expected input to be sequential daily weather'
            )
        return None
    ...
    r[["SOLRAD", "SUNRISE", "SUNSET"]] = r.apply(
        lambda row: util.sun(
            latitude, longitude, row["MON"], row["DAY"], row["HOUR"], timezone
        ),
        axis=1,
        result_type="expand",
    )
