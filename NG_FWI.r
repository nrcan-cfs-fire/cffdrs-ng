...
hourly_gfmc <- function(temp, rh, wind, rain, lastmc, solrad, time) {
  ...
  # fuel temp from CEVW
  tf <- temp + 17.9 * solrad * exp(-0.034 * wind)
  if (tf > temp) {
  ...
  if (rain != 0) {
    # /* old routine*/
    # /*     mo+=rain*rf*exp(-100.0/(251.0-mo))*(1.0-exp(-6.93/rain))*/
  ...
  # /*GRASS EMC*/
  ed <- 1.62 * rhf^0.532 + (13.7 * exp((rhf - 100) / 13.0)) + 0.27 * (26.7 - tf) * (1.0 - 1.0 / exp(0.115 * rhf))
  moed <- mo - ed
  # /*GRASS EMC*/
  ew <- 1.42 * rhf^0.512 + (12.0 * exp((rhf - 100) / 18.0)) + 0.27 * (26.7 - tf) * (1.0 - 1.0 / exp(0.115 * rhf))
  moew <- mo - ew
