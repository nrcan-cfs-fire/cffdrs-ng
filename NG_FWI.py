...
def fwi_calc(isi, bui):
    bb = (0.1 * isi * (1000 / (25 + 108.64 / exp(0.023 * bui)))) if bui > 80 else (0.1 * isi * (0.626 * (bui ** 0.809) + 2))
...
def hourly_ffmc(weatherstream, ffmc_old=FFMC_DEFAULT):
    def hffmc(temp, rh, ws, prec, ffmc_old):
        ...
        mr = mo if 0 == prec else (mo + 42.5 * prec * exp(-100 / (251 - mo)) * (1 - exp(-6.93 / prec))) if mo <= 150 else (mo + 42.5 * prec * exp(-100 / (251 - mo)) * (1 - exp(-6.93 / prec)) + 0.0015 * ((mo - 150) ** 2) * (prec ** 0.5))
        mr = min(250, mr)
        ...
        k1 = 0.424 * (1 - ((100 - rh) / 100) ** 1.7) + 0.0694 * (ws ** 0.5) * (1 - ((100 - rh) / 100) ** 8)
...
def _hffmc(w, ffmc_old):
    ...
    for_ffmc['FFMC_MULTIPLIER'] = for_ffmc.apply(lambda row: 0 if 0.5 >= row['SUM_PREC'] else (row['SUM_PREC'] - 0.5) / row['SUM_PREC'], axis=1)
    for_ffmc['PREC'] = for_ffmc.apply(lambda row: row['PREC'] * row['FFMC_MULTIPLIER'], axis=1)
...
def _hdmc(w, dmc_old):
    def hourly_dmc(t, rh, ws, rain, mon, lastdmc, DryFrac, rain24, DELTA_MCrain, tnoon, rhnoon):
    ...
    dmc['VPD'] = dmc.apply(lambda row: vpd(row['TEMP'], row['RH']) if row['SUNRISE'] <= row['HR'] and row['HR'] <= row['SUNSET'] else 0.0, axis=1)
    dmc = pd.merge(
        dmc,
        pd.DataFrame({
    ...
    dmc['DRYFRAC'] = dmc.apply(lambda row: ((1 / (row['SUNSET'] - row['SUNRISE'])) if 0 == row['VPD24'] else (row['VPD'] / row['VPD24'])) if row['SUNRISE'] <= row['HR'] <= row['SUNSET'] else 0.0, axis=1)
    lastdmc = dmc_old
    dates = w['DATE'].unique()
    result = []
    for d in dates:
        ...
        for r in for_date.itertuples():
            lastdmc = hourly_dmc(r.TEMP, r.RH, r.WS, r.PREC, r.MON, lastdmc, r.DRYFRAC, r.RAIN24, DELTA_mcddmcrain24, tnoon, rhnoon)
...
def _hdc(w, dc_old):
    ...
    if 'SUNRISE' not in dc.columns:
        raise RuntimeError('SUNRISE column required')
    dc['VPD'] = dc.apply(lambda row: vpd(row['TEMP'], row['RH']) if row['SUNRISE'] <= row['HR'] and row['HR'] <= row['SUNSET'] else 0.0, axis=1)
    dc = pd.merge(
        dc,
        pd.DataFrame({
    ...
    dc['DRYFRAC'] = dc.apply(lambda row: ((1 / (row['SUNSET'] - row['SUNRISE'])) if 0 == row['VPD24'] else (row['VPD'] / row['VPD24'])) if row['SUNRISE'] <= row['HR'] <= row['SUNSET'] else 0.0, axis=1)
    # #>>>
...
def hourly_gfmc(temp, rh, wind, rain, lastmc, solrad, time):
    ...
    rhf = (rh * 6.107 * 10.0 ** (7.5 * temp / (temp + 237.0)) / (6.107 * 10.0 ** (7.5 * tf / (tf + 237.0)))) if tf > temp else rh
    if rain != 0:
...
def grass_isi(wind, mc, cur):
    fw = ((0.054 + 0.209 * wind) * 16.67) if wind < 5 else ((1.1 + 0.715 * (wind - 5.0) * 0.844) * 16.67)
    if mc < 12:
        fm = exp(-0.108 * mc)
...
def _stnHFWI(w, timezone, ffmc_old, dmc_old, dc_old, percent_cured):
    ...
    solrad, sunrise, sunset = util.sun(row['LAT'], row['LONG'], row['MON'], row['DAY'], row['HR'], timezone)
    #print(solrad, sunrise, sunset)
    r[['SOLRAD', 'SUNRISE', 'SUNSET']] = r.apply(lambda row: util.sun(row['LAT'], row['LONG'], row['MON'], row['DAY'], row['HR'], timezone), axis=1, result_type='expand')
    ...
    r = pd.merge(
        r,
        pd.DataFrame({
            'MIN_RH': r.groupby('DATE')['RH'].min()
        }),
        on=['DATE']
    )
    r['MIN_RH'] = r.apply(lambda row: min(100, max(0, row['MIN_RH'])), axis=1)
    r['SOLPROP'] = r.apply(lambda row: (1.27 - 0.0111 * row['MIN_RH']) if row['MIN_RH'] > 30 else 1, axis=1)
    ...
    r['GFMC'] = r.apply(lambda row: 59.5 * (250 - row['MCGMC']) / (147.2772277 + row['MCGMC']), axis=1)
    r['GSI'] = r.apply(lambda row: grass_isi(row['WS'], row['MCGMC'], percent_cured), axis=1)
...
# @param     percent_cured   Grass curing (percent, 0-100)
# @return                    hourly values FWI and weather stream
def hFWI(weatherstream, timezone, ffmc_old=FFMC_DEFAULT, dmc_old=DMC_DEFAULT, dc_old=DC_DEFAULT, percent_cured=PERCENT_CURED):
    ...
    if not had_date:
        wx['DATE'] = wx.apply(lambda row: f'{row["YR"]:04d}-{row["MON"]:02d}-{row["DAY"]:02d}', axis=1)
    if not had_timestamp:
        wx['TIMESTAMP'] = wx.apply(lambda row: datetime.datetime(row['YR'], row['MON'], row['DAY'], row['HR'], row['MINUTE']), axis=1)
    #########################################
    # PROCESS HERE
    #########################################
    results = None
