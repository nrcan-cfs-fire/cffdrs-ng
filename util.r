...
getSunlight <- function(dates, temprange, timezone, latitude, longitude) {
  ...
  df[, solradExt := 1.367 * cos(zenith)]
  # Daily total of Extra. Solar Rad in kJ m-2 day-1
  df[, solradExtDaySum := sum(solradExt) * 3600, by = c("d")]
  # Daily surface Solar Rad in kJ m-2 day-1
  df[, solradDaySum := 0.11 * solradExtDaySum * temprange^0.59]
  # hourly surface solar rad in kW m-2
  df[, solradHar := cos(zenith) / sum(cos(zenith)) * solradDaySum / 3600, by = c("d")]
  # Atmospheric Transmissivity
  df[, trans := solradDaySum / solradExtDaySum, by = c("d")]
  ###########################################################################################
  ###########################################################################################
...
getPET <- function(dmc) {
  C_TO_K <- 273
  # latent heat of vaporisation (kJ/kg) (from https://link.springer.com/referenceworkentry/10.1007%2F978-90-481-2642-2_327)
  LofVap <- 2260
  # Psychometric constant (Pa/C) (taken from the 500 m elevation value from Table 2.2 of https://www.fao.org/3/x0490e/x0490e0j.htm#annex%202.%20meteorological%20tables)
  psychro <- 64
  # Stefan Boltzmann constant (kW m-2 K-4 )
  SB <- 5.67 * (10^-11)
  # density of air (kg/m^3)
  RHO_A <- 1.09266
  # specific heat capacity of air (kJ kg-1 K-1) From Stull (1988)
  C_A <- 1.00467
  # von Karman constant (from https://glossary.ametsoc.org/wiki/Von_k%C3%A1rm%C3%A1n%27s_constant)
  k <- 0.40
  # m HOW TO GET HEIGHT FROM THE ORIGINAL DATAFRAME???
  wind_height <- 1.5
  # m displacement height THIS IS CURRENTLY AN UNINFORMED GUESS
  disp_height <- 0
  # m roughness length Taken from the sharpsands study
  z_o <- 0.01
  # emissivity of vegetation used for downwelling canopy longwave and upwelling surface longwave
  e_veg <- 0.95
  albedo <- 0.23
  time_res <- 3600
  ...
  # ## PM - PET MODEL
  #
  # Sat vapour pressure (Pa) (factor of 1000 required to convert from kPa to Pa)
  # from: https://www.fao.org/3/x0490e/x0490e0j.htm#annex%202.%20meteorological%20tables
  dmc[, SVP_sub := 0.6108 * exp(17.27 * TEMP_sub / (237.3 + TEMP_sub)) * 1000]
  # slope of VP curve (Pa/C)
  # from: https://www.fao.org/3/x0490e/x0490e0j.htm#annex%202.%20meteorological%20tables
  dmc[, satSlope_sub := 4098 * (SVP_sub) / (TEMP_sub + 237.3)^2]
  # vapour pressure (Pa)
  dmc[, VP_sub := (RH_sub * SVP_sub) / 100]
  dmc[, VPD_sub := SVP_sub - VP_sub]
  # Sat vapour pressure (Pa) (factor of 1000 required to convert from kPa to Pa)
  # from: https://www.fao.org/3/x0490e/x0490e0j.htm#annex%202.%20meteorological%20tables
  dmc[, SVP_open := 0.6108 * exp(17.27 * TEMP / (237.3 + TEMP)) * 1000]
  # slope of VP curve (Pa/C)
  # from: https://www.fao.org/3/x0490e/x0490e0j.htm#annex%202.%20meteorological%20tables
  dmc[, satSlope_open := 4098 * (SVP_open) / (TEMP + 237.3)^2]
  # vapour pressure (Pa)
  dmc[, VP_open := (RH * SVP_open) / 100]
  dmc[, VPD_open := SVP_open - VP_open]
  # clear sky emissivity
  # from: https://doi.org/10.1002/hyp.6383
  # (factor of 1/100 for converting VP from Pa to mb)
  dmc[, e_clear := 1.24 * (VP_open / 100 / (TEMP + C_TO_K))^(1 / 7)]
  # aerodynamic resistance (s m-1)
  # from equation 4 of https://www.fao.org/3/x0490E/x0490e06.htm
  dmc[, ra_sub := (log((wind_height - disp_height) / z_o))^2 / (k^2 * WS_sub)]
  # Canopy downwelling longwave (kW m-2)
  dmc[, Canopy_L := e_veg * SB * ((TEMP_sub + C_TO_K)^4)]
  # upwelling longwave (W m-2)
  dmc[, Lup := e_veg * SB * ((TEMP_sub + C_TO_K)^4)]
  # Daily mean RH
  dmc[, MEANRH := mean(RH), by = c("DATE")]
  # increase in atm emissivity (from the clear sky case) due to cloud emissions.
  # From second equation in Table 1 of https://doi.org/10.1002/hyp.6383
  dmc[, F.daily := 0.87 + 0.64 * MEANRH / 100 - 0.46 * TRANS]
  dmc[, e.clear.daily := mean(e_clear), by = c("YR", "MON", "DAY", "LAT")]
  # canopy transmission of solar radiation (frac)
  dmc[, T.daily := mean(subSolar) / mean(SOLRADHAR), by = c("YR", "MON", "DAY", "LAT")]
  # downwelling sky longwave (kW m-2)
  dmc[, Lsky := e.clear.daily * F.daily * SB * ((TEMP + C_TO_K)^4)]
  # subcanopy downwelling longwave (kW m-2)
  dmc[, Ldown := T.daily * Lsky + (1 - T.daily) * Canopy_L]
  # net radiation (kW m-2).
  dmc[, `Q*` := subSolar - (subSolar * albedo) + (Ldown - Lup)]
  dmc[, radterm_sub := satSlope_sub * `Q*`]
  dmc[, vpdterm_sub := RHO_A * C_A * (VPD_sub) / ra_sub]
  # potential evapotransp (kg/(m2·s))
  # from https://en.wikipedia.org/wiki/Penman_equation
  dmc[, PET_sub := (radterm_sub + vpdterm_sub) / (LofVap * (satSlope_sub + psychro))]
  # potential evapotransp (mm/time step)
  dmc[, PET_mm_sub := PET_sub * time_res]
