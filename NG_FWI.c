...
void main(int argc, char* argv[])
{
  ...
  while (err > 0)
  {
    ...
    {
      /* This loads up 24 hours at a time  ...need to for this version*/
    ...
    for (h = 0, rain24 = 0.0, minRH = 100.0, Wdmc24 = 0.0, Wdc24 = 0.0; h < 24; h++)
    {
      ...
      {
        Wdmc24 += DMCdryingweight(atemp[h], arh[h], aws[h]);
      }
      if (h >= sunrise && h <= sunset)
      {
        Wdc24 += DCdryingweight(atemp[h], arh[h], aws[h]);
      }
    }

    if (rain24 > 0.5)
    {
      /* so we can use the old hourly routine directly*/
      ffmcRF = (rain24 - 0.5) / rain24;
    }
    else
    {
      ffmcRF = 0.0;
    ...
    for (h = 0; h < 24; h++)
    {
      ...
      if (Wdmc24 > 0)
      {
        if (h >= sunrise && h <= sunset)
        {
          dmcDryFrac = DMCdryingweight(atemp[h], arh[h], aws[h]) / Wdmc24;
        }
        else
        {
          dmcDryFrac = 0.0;
      ...
      else
      {
        ...
        if (h >= sunrise && h <= sunset)
        {
          /* is VPD is 0 all day long --rh=100 all day -- then set drying fraction to be uniform */
          dmcDryFrac = 1 / daylength;
        }
        else
        {
          dmcDryFrac = 0.0;
      ...
      if (Wdc24 > 0)
      {
        if (h >= sunrise && h <= sunset)
        {
          dcDryFrac = DCdryingweight(atemp[h], arh[h], aws[h]) / Wdc24;
        }
        else
        {
          dcDryFrac = 0.0;
...
float hourly_DC(float t, float rh, float ws, float rain, float lastdc, int mon, float rain24, float dryfrac, float DELTArain24, float temp12)
{
  ...
  if (rain > 0 && DELTArain24 < 0.0)
  {
    ...
    lastdc += DELTArain24 * (rain / rain24);
  }
